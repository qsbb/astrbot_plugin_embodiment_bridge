...
6. 公网必须部署 Quest 信任的 HTTPS。受控私网可显式启用 `allow_private_http_pairing`，但只接受私网 IP 字面量；内置 listener 不终止公网 TLS。
7. 在 AstrBot 已安装插件页面打开本插件的「Quest 快速绑定」Page。
...
Bridge 就绪后，Page 只显示「生成绑定二维码」。点击后显示二维码、6 位短码、倒计时和 Quest 兑换状态。配对默认 120 秒有效；关闭或重载插件会使未完成的配对立即失效。
...
- 快速绑定 Page 不预先收集 Quest IP；高熵 QR token 依靠单次使用和短有效期保护，6 位短码同时受每来源和全局限速。旧完整创建请求仍可显式提供 `expected_remote_ip` 以增加来源绑定。
...
- 匿名：`POST /api/v1/plugins/extensions/astrbot_plugin_quest_avatar_bridge/pairing/exchange`。
...
配对完成后，Unity 必须对 8520 上的正常接口继续发送它实际获得的 `Authorization: Bearer ...` 和 `X-Quest-Avatar-Key`。listener 不添加、替换或修复认证头。
