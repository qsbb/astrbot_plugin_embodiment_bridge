...
- 管理页面：“具身服务控制台”按 `运行 / 设备与绑定 / 对话与模型 / 人格` 分区，页头有状态 chip、能力健康折叠成一行摘要（异常自动展开）；“设备与绑定”提供四步绑定向导（服务地址 → 平台实例 → 基础身份 → 快速绑定，缺失项可一键跳转），运行区再分 `服务 / 集成 / 诊断` 三个子页签并在出现根因时自动定位；“具身客户端快速绑定”生成一次性二维码与 6 位短码完成配对。
...
覆盖快照由临在插件数据目录中原子保存；核不可用、契约不兼容、revision 冲突或快照损坏时，临继续使用自身配置。契约声明 `read_native` 与 `write_native`：核可读取临的原生配置现值，也可把当前生效值一键固化进临自身的 AstrBot 配置（写前自动生成 `native-backup-<UTC 时间戳>.json` 备份），核临时掉线时临仍按固化后的配置运行。关闭统一接管后无需重启即可恢复原生配置。
