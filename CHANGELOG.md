...
- 接入核统一模型路由：对话（含直管主链 ``quest_enriched_pipeline``）、STT、TTS 三条路径均按「本地显式配置（有效时）→ 核路由 → AstrBot 原生」取 provider；核路由命中时消费其模型，本地配置有效时不附带核模型。

### 修复

- 插件实例解析补上 AstrBot 官方接口回退（``get_registered_star`` → ``StarMetadata.star_cls``），修复官方环境下拿不到核实例、核路由从未生效的问题。
- 主对话链不再要求必须配置本地 ``chat_provider_id``：本地未配置或已失效时会先尝试核路由，都没有才报原有的「未配置」错误。

## 1.7.7 - 2026-09-19

### 变更

- 页面文案清理：能力说明不再与悬停提示逐字重复（删 title 保留可见 hint，窄屏同样可见）；删去介绍界面交互方式的元注释；自动刷新周期自述与实现对齐（10 秒）。
- 实现说明折叠：对话链路与快速动作决策的原理说明收进「实现说明」可展开区域，正文只留操作指引。
- 断行兜底：行内操作按钮组允许换行；配对按钮文案缩短。
- 提示与注释去修订痕迹：错误提示不再附带无关链路声明；模块说明改为当前态陈述（含修正内部两处互相矛盾的表述）。
...
- 11 个布尔开关（异步快速动作 / 声与 AstrBot TTS / 任务链回退与工具过滤 / 知识与环境注入 / 诊断日志与显示项）改用 series.ui 统一滑块开关。
...
- 服务地址、运行状态、平台实例、版本与能力健康合并为「运行摘要」卡；页头压缩为单状态 chip（健康显示「8 项正常」）。
- 二级导航前置并 sticky，390px 首屏可见；修复对话页签在双列网格中的错位。
- 公网地址迁入「平台与集成」，诊断下沉且异常自动展开；人格区改主从布局 + sticky 操作栏。
...
- 新增设备绑定四步 stepper（服务地址 → 平台实例 → 基础身份 → 快速绑定），缺失项可一键跳转并聚焦。
- 运行区拆分服务 / 集成 / 诊断三个子页签，出现根因时一次性自动定位；服务卡补齐内边距。
- 对话与模型的两张卡片在桌面并排等高，主按钮不再被拉伸为整行；人格编辑器为 sticky 操作栏预留空间。
- 术语汉化（对话模型服务商、语音识别模型服务商、共享消息链路、说话/待机、授权身份）。

## 1.7.1 - 2026-09-13

### 变更

- 修复临页面二级 tab 网格错位，主导航前置并 sticky，保持绑定、诊断和人格流程兼容；补齐共享 UI 布局组件。

## 1.7.0 - 2026-09-13

### 新增

- 全面接入 `series.ui@1.0` Glass Aurora 组件库，统一服务控制、能力状态、人格编辑、二维码与诊断时间线的视觉层级。
...
- 接入 `series.module@1.0` 与 `series.webui@2.0`，临服务状态支持有界、可取消、脱敏的 SSE 实时流。

### 修复

- 修正模块契约中误写的 JSON 布尔字面量；同步 operator Page 缓存戳和版本回归基线。
...
- operator 页面补齐后端既有能力（B9，用户拍板）：「运行」标签新增「配对地址」
  面板——监听器公网地址与临桥服务公网地址两个编辑框（URL 校验复用既有
  归一化器，空串即清除，保存即热更新监听器公告地址与 exchange URL，
  无需重启）；端点 pairing/public-url-settings GET/POST（dashboard 鉴权）
...
- 接入 `series.control@1.0`，仅开放诊断、会话并发、事件队列、音频边界、分块和心跳等非秘密运行策略。
- 保留临自身配置所有权；核覆盖通过独立原子快照保存，关闭接管、契约不可用或快照损坏时回退原生配置。
- 新增 revision 冲突、失败回滚、字段白名单和秘密字段排除测试。
...
- 统一插件入口版本、operator 页面缓存参数与发布元数据，避免升级后继续使用旧页面资源。

## 1.0.17 - 2026-08-18

### Fixed

- 动作解析现在保留“不要回复文字，只挥手/下蹲”等合法 action-only 指令，同时继续拒绝真正的动作否定。
