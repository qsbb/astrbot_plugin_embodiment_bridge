...
QUEST_BINDING_CONTROL_NAME = "identity.quest_binding_control"
QUEST_BINDING_CONTROL_VERSION = "1.0"
...
_QUEST_BINDING_RESPONSE_FIELDS = (
    "contract_version",
    "status",
    "reason",
    "updated",
...
_QUEST_BINDING_RESPONSE_KEYS = set(_QUEST_BINDING_RESPONSE_FIELDS)
...
class IdentityControlPlaneAdapter:
    ...
    async def upsert_quest_read_only_binding(
        self,
        *,
        api_principal_digest: str,
        client_id: str,
        ...
        try:
            value = provider.upsert_quest_binding(request)
            if inspect.isawaitable(value):
                value = await asyncio.wait_for(
                    value,
                    timeout=CONTROL_PLANE_TIMEOUT_SECONDS,
            ...
            raise IdentityControlPlaneError(
                "quest_binding_control_timeout",
                "Quest 只读身份控制面保存超时",
            ) from exc
        except Exception as exc:
            raise IdentityControlPlaneError(
                "quest_binding_control_error",
                "Quest 只读身份控制面保存失败",
            ) from exc
        validated = self._validate_quest_binding_response(result)
        if (
            validated["status"] != "saved"
            or validated["updated"] is not True
            or validated["authorized"] is not True
            ...
            raise IdentityControlPlaneError(
                str(validated["reason"] or "quest_binding_control_rejected"),
                "Quest 只读身份控制面拒绝了绑定",
            )
        return {
            "source": "identity_guardian",
    ...
    @staticmethod
    def _require_quest_binding_contract(provider: Any) -> None:
        try:
            contract = provider.quest_binding_control_contract()
        except (AttributeError, RuntimeError, TypeError, ValueError) as exc:
            raise IdentityControlPlaneError(
                "quest_binding_control_incompatible",
                "已安装的“序”不提供兼容的 Quest 只读绑定契约",
            ) from exc
        compatible = bool(
            isinstance(contract, dict)
            and contract.get("name") == QUEST_BINDING_CONTROL_NAME
            and contract.get("version") == QUEST_BINDING_CONTROL_VERSION
            and contract.get("plugin") == IDENTITY_PLUGIN_NAME
            and tuple(contract.get("capabilities") or ())
            == (
            ...
            and tuple(contract.get("methods") or ())
            == ("upsert_quest_binding", "revoke_quest_binding")
            and contract.get("privacy") == "counts_only"
            and contract.get("principal_storage") == "sha256_digest_only"
            and contract.get("owner_users_mutated") is False
            and contract.get("natural_person_grants_permission") is False
            ...
            and tuple(contract.get("revoke_request_fields") or ())
            == ("api_principal_digest", "client_id")
            and tuple(contract.get("response_fields") or ())
            == _QUEST_BINDING_RESPONSE_FIELDS
        )
        if not compatible:
            raise IdentityControlPlaneError(
        ...
        *,
        ...
        if provider is None:
            return {
                ...
                "authoritative": False,
                "status": "revoked",
                "reason": "revoked_bridge_local_fallback",
                "updated": True,
                "authorized": False,
                "config_writable": True,
        ...
        try:
            value = provider.revoke_quest_binding(request)
            if inspect.isawaitable(value):
                value = await asyncio.wait_for(
                    value,
                    timeout=CONTROL_PLANE_TIMEOUT_SECONDS,
        ...
        validated = self._validate_quest_binding_response(result)
        if validated["status"] != "revoked" or validated["authorized"] is not False:
            raise IdentityControlPlaneError(
                str(validated["reason"] or "quest_binding_control_revoke_rejected"),
                "Quest 只读身份控制面拒绝了撤销",
            )
        return {
            "source": "identity_guardian",
    ...
    @staticmethod
    def _validate_quest_binding_response(value: Any) -> dict[str, Any]:
        if not isinstance(value, dict) or set(value) != _QUEST_BINDING_RESPONSE_KEYS:
            raise IdentityControlPlaneError(
                "quest_binding_control_invalid_response",
                "Quest 只读身份控制面返回了无效响应",
            )
        valid = bool(
            value.get("contract_version") == QUEST_BINDING_CONTROL_VERSION
            and value.get("status")
            in {"saved", "revoked", "rejected", "unavailable", "error"}
            and isinstance(value.get("reason"), str)
            and isinstance(value.get("updated"), bool)
            and isinstance(value.get("authorized"), bool)
            and isinstance(value.get("config_writable"), bool)
            and isinstance(value.get("read_only_binding_count"), int)
            and value["read_only_binding_count"] >= 0
            and value.get("grants_owner") is False
            and value.get("grants_platform_action") is False
        )
        if not valid:
            raise IdentityControlPlaneError(
