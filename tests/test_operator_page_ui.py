...
def test_operator_page_is_discoverable_and_uses_page_bridge() -> None:
    ...
    assert '<script type="module" src="./app.js?v=1.4.0-8"></script>' in html
    assert '<link rel="stylesheet" href="./style.css?v=1.4.0-8" />' in html
    assert html.index("bridge-sdk.js") < html.index("./app.js?v=1.4.0-8")
...
def test_operator_page_exposes_only_safe_model_and_identity_workflows() -> None:
    ...
    assert "pairing-listener-public-url" in html
    assert "pairing-public-url" in html
    assert "save-public-url-button" in html
    assert "公网访问地址" in html
    assert 'endpoint: "pairing/public-url-settings"' in js
    assert "function renderPublicUrlSettings" in js
    assert '"public-url": ["public-url-status", "配对地址读取失败，可单独重试。"]' in js
